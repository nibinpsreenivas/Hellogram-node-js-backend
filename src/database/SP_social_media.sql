...
BEGIN
    SELECT 
        img.post_uid AS post_uid, 
        pos.is_comment, 
        pos.type_privacy, 
        pos.created_at, 
        pos.person_uid, 
        pos.post_emotion,
        MAX(us.username) AS username,  -- Use an aggregate function
        per.image AS avatar, 
    ...
    FROM 
        images_post img
    INNER JOIN 
        posts pos ON img.post_uid = pos.uid
    INNER JOIN 
        comments co ON co.post_uid = pos.uid
    INNER JOIN 
        users us ON us.person_uid = pos.person_uid
    INNER JOIN 
        person per ON per.uid = pos.person_uid
    LEFT JOIN 
        friends f ON f.friend_uid = per.uid
    WHERE 
        f.person_uid = ID OR pos.person_uid = ID
    GROUP BY 
        img.post_uid, 
        pos.is_comment, 
        pos.type_privacy, 
